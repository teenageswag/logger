  ...
  void write(const LogMessage &msg) override {
    ...
    std::string final_msg =
        std::format("{}[{:02}:{:02}:{:02}] [{}] [{}:{}] {}{}\n",
                    ...
                    msg.loc.line(), msg.text, reset_color);
...
inline void log_impl(Level level, const std::source_location &loc,
                     std::string_view text) {
  if (level < Logger::instance().get_level())
    return;
...
struct log {
  ...
  static void add_file_sink(const std::filesystem::path &path,
                            size_t max_size = 5 * 1024 * 1024) {
    log_detail::Logger::instance().add_sink(
        std::make_unique<log_detail::FileSink>(path, max_size));
  }

  static void add_console_sink(ConsoleConfig cfg = {}) {
    log_detail::Logger::instance().remove_console_sinks();
    log_detail::Logger::instance().add_sink(
        std::make_unique<log_detail::ConsoleSink>(std::move(cfg)));
  }
  ...
  static void
  trace(log_detail::log_format_string<std::type_identity_t<Args>...> fmt,
        Args &&...args) {
    if constexpr (LOG_ACTIVE_LEVEL <= Level::TRACE) {
      if (Level::TRACE < log_detail::Logger::instance().get_level())
        return;
      thread_local std::string tls_buffer;
      tls_buffer.clear();
      std::format_to(std::back_inserter(tls_buffer), fmt.fmt,
  ...
  template <typename... Args>
  static void
  debug(log_detail::log_format_string<std::type_identity_t<Args>...> fmt,
        Args &&...args) {
    if constexpr (LOG_ACTIVE_LEVEL <= Level::DEBUG) {
      if (Level::DEBUG < log_detail::Logger::instance().get_level())
        return;
      thread_local std::string tls_buffer;
      tls_buffer.clear();
      std::format_to(std::back_inserter(tls_buffer), fmt.fmt,
  ...
  template <typename... Args>
  static void
  info(log_detail::log_format_string<std::type_identity_t<Args>...> fmt,
       Args &&...args) {
    if constexpr (LOG_ACTIVE_LEVEL <= Level::INFO) {
      if (Level::INFO < log_detail::Logger::instance().get_level())
        return;
      thread_local std::string tls_buffer;
      tls_buffer.clear();
      std::format_to(std::back_inserter(tls_buffer), fmt.fmt,
  ...
  template <typename... Args>
  static void
  warn(log_detail::log_format_string<std::type_identity_t<Args>...> fmt,
       Args &&...args) {
    if constexpr (LOG_ACTIVE_LEVEL <= Level::WARN) {
      if (Level::WARN < log_detail::Logger::instance().get_level())
        return;
      thread_local std::string tls_buffer;
      tls_buffer.clear();
      std::format_to(std::back_inserter(tls_buffer), fmt.fmt,
  ...
  template <typename... Args>
  static void
  error(log_detail::log_format_string<std::type_identity_t<Args>...> fmt,
        Args &&...args) {
    if constexpr (LOG_ACTIVE_LEVEL <= Level::ERROR) {
      if (Level::ERROR < log_detail::Logger::instance().get_level())
        return;
      thread_local std::string tls_buffer;
      tls_buffer.clear();
      std::format_to(std::back_inserter(tls_buffer), fmt.fmt,
